...
async function createCadStudioServer(options) {
  ...
  const server = new McpServer(
    { name: "mcp-cad-studio", version: "0.2.2" },
    {
...
function valueAfter(args, index, option) {
  ...
  if (!value2 || value2.startsWith("--")) throw new Error(`${option} needs a value`);
  return value2;
}
function parseCli(args) {
  const options = {
    ...
    port: 8787,
    https: false,
    dataFile: join2(homedir(), ".mcp-cad-studio", "models.json")
  };
